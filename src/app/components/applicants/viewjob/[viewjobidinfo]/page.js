...
import styles from "../../../employer/jobsPosted/[jobInfo]/jobInfo.module.css";
import jobActive from "../../../employer/jobsActive/jobsActive.module.css";
import { getSingleJob } from "@/app/redux/employer/jobPostedSlice/jobPostedSlice";
import { useDispatch, useSelector } from "react-redux";
import stylesJob from "../../../employer/jobsPosted/jobsPosted.module.css";
...
const JobInfo = ({ params }) => {
  ...
  const { viewjobidinfo } = params;
  const { id, isLoading } = useSelector((state) => state.jobPost);
  console.log(id, "id of view job");
  const singleJob = id;
  const checkValidationDate = date > new Date(singleJob?.applicationDeadline);
  useEffect(() => {
    dispatch(getSingleJob(viewjobidinfo));
  }, []);

  console.log(singleJob, "single job in view job");
